...
describe "RBM SDR migration" do
  def run_migration_script(script)
    ...
    Dir.mktmpdir do |dir|
      ...
      initialize = 'require "db"; Sequel.extension(:migration); Sequel::Migrator.run(DB, "db/migrate", target: 39)'
  ...
  it "completes setup with an unavailable cache and preserves repaired history through recovery" do
    run_migration_script(<<~'RUBY')
      rows = [
        { provider: "RBM", date: "2026-01-01", base: "SDR", quote: "MWK", mid: 2200.5 },
        { provider: "RBM", date: "2026-01-02", base: "USD", quote: "MWK", mid: 1700.0 },
        { provider: "RBM", date: "2026-01-03", base: "SDR", quote: "MWK", mid: 2210.0 },
        { provider: "RBM", date: "2026-01-03", base: "XDR", quote: "MWK", mid: 2210.0 },
        { provider: "RBM", date: "2026-01-04", base: "XDR", quote: "MWK", mid: 2215.0 },
        { provider: "RBM", date: "2026-01-05", base: "MWK", quote: "SDR", mid: 0.00045 },
        { provider: "RBM", date: "2026-01-06", base: "SDR", quote: "MWK", mid: nil, bid: 2200.0, ask: 2220.0 },
      ...
      expected = raw.map do |row|
        row[:provider] == "RBM" ? row.transform_values { |value| value == "SDR" ? "XDR" : value } : row
      end.uniq.sort_by { |row| row.values_at(:provider, :date, :base, :quote) }
      CurrencySummary.refresh(DB, ["SDR", "XDR", "MWK", "USD", "CAD"])
      [:weekly_rates, :monthly_rates].zip([Bucket.week, Bucket.month]).each do |table, bucket|
        ...
        DB[table].insert(bucket_date: "2025-12-01", provider: "RBM", base: "SDR", quote: "MWK", rate: 999)
      end
      ...
      abort "migration incomplete" unless DB[:schema_info].get(:version) >= 40
      abort "changed native observations" unless DB[:rates].order(:provider, :date, :base, :quote).all == expected
      abort "still unknown SDR" if Provider["RBM"].unknown_currencies.include?("SDR")
      abort "lost other provider exclusion" unless Provider["BOC"].unknown_currencies.include?("SDR")
      ["XDR", "MWK"].each do |code|
        coverage = DB[:currency_coverages].where(provider_key: "RBM", iso_code: code).first
      ...
      [BlendedWeeklyRate, BlendedMonthlyRate].each do |model|
        ...
        abort "retained RBM SDR base rollup" unless model.source.where(provider: "RBM", base: "SDR").empty?
        abort "retained RBM SDR quote rollup" unless model.source.where(provider: "RBM", quote: "SDR").empty?
      end
      ...
      expected_average = DB[:rates].where(provider: "RBM", base: "XDR", quote: "MWK").avg(:rate)
      actual_average = DB[:monthly_rates].where(provider: "RBM", base: "XDR", quote: "MWK").get(:rate)
  ...
  it "rolls back conflicting native components even when their effective rates match" do
    run_migration_script(<<~RUBY)
      DB[:rates].multi_insert([
        { provider: "RBM", date: "2026-01-01", base: "SDR", quote: "MWK", mid: 2200, bid: nil, ask: nil },
        { provider: "RBM", date: "2026-01-02", base: "SDR", quote: "MWK", mid: 2210, bid: nil, ask: nil },
        { provider: "RBM", date: "2026-01-02", base: "XDR", quote: "MWK", mid: nil, bid: 2200, ask: 2220 },
      ])
      DB[:blended_rates].insert(date: "2026-01-01", quote: "MWK", rate: 1700)
      ...
      abort "migration marked complete" unless DB[:schema_info].get(:version) == 39
      abort "invalidated blends after failure" unless DB[:blended_rates].count == 1
      DB.disconnect
    RUBY
  end

  it "leaves databases without RBM SDR history alone" do
    run_migration_script(<<~RUBY)
      DB[:blended_rates].insert(date: "2026-01-01", quote: "MWK", rate: 1700)
      Sequel::Migrator.run(DB, "db/migrate")
