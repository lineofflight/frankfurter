...
describe "BNM SDR migration" do
  def run_migration_script(script)
    ...
    Dir.mktmpdir do |dir|
      ...
      initialize = 'require "db"; Sequel.extension(:migration); Sequel::Migrator.run(DB, "db/migrate", target: 38)'
  ...
  it "completes setup with an unavailable cache and preserves repaired history through recovery" do
    run_migration_script(<<~'RUBY')
      rows = [
        { provider: "BNM", date: "2026-01-01", base: "SDR", quote: "MYR", mid: 5.3538 },
        { provider: "BNM", date: "2026-01-02", base: "USD", quote: "MYR", mid: 3.9300 },
        { provider: "BNM", date: "2026-01-03", base: "SDR", quote: "MYR", mid: 5.3795 },
        { provider: "BNM", date: "2026-01-03", base: "XDR", quote: "MYR", mid: 5.3795 },
        { provider: "BNM", date: "2026-01-04", base: "XDR", quote: "MYR", mid: 5.3877 },
        { provider: "BNM", date: "2026-01-05", base: "MYR", quote: "SDR", mid: 0.1858 },
        { provider: "BNM", date: "2026-01-06", base: "SDR", quote: "MYR", mid: nil, bid: 5.3500, ask: 5.3600 },
      ...
      expected = raw.map do |row|
        row[:provider] == "BNM" ? row.transform_values { |value| value == "SDR" ? "XDR" : value } : row
      end.uniq.sort_by { |row| row.values_at(:provider, :date, :base, :quote) }
      CurrencySummary.refresh(DB, ["SDR", "XDR", "MYR", "USD", "CAD"])
      [:weekly_rates, :monthly_rates].zip([Bucket.week, Bucket.month]).each do |table, bucket|
        ...
        DB[table].insert(bucket_date: "2025-12-01", provider: "BNM", base: "SDR", quote: "MYR", rate: 999)
      end
      ...
      abort "migration incomplete" unless DB[:schema_info].get(:version) >= 39
      abort "changed native observations" unless DB[:rates].order(:provider, :date, :base, :quote).all == expected
      abort "still unknown SDR" if Provider["BNM"].unknown_currencies.include?("SDR")
      abort "lost other provider exclusion" unless Provider["BOC"].unknown_currencies.include?("SDR")
      ["XDR", "MYR"].each do |code|
        coverage = DB[:currency_coverages].where(provider_key: "BNM", iso_code: code).first
      ...
      [BlendedWeeklyRate, BlendedMonthlyRate].each do |model|
        ...
        abort "retained BNM SDR base rollup" unless model.source.where(provider: "BNM", base: "SDR").empty?
        abort "retained BNM SDR quote rollup" unless model.source.where(provider: "BNM", quote: "SDR").empty?
      end
      ...
      expected_average = DB[:rates].where(provider: "BNM", base: "XDR", quote: "MYR").avg(:rate)
      actual_average = DB[:monthly_rates].where(provider: "BNM", base: "XDR", quote: "MYR").get(:rate)
  ...
  it "rolls back conflicting native components even when their effective rates match" do
    run_migration_script(<<~RUBY)
      DB[:rates].multi_insert([
        { provider: "BNM", date: "2026-01-01", base: "SDR", quote: "MYR", mid: 5.35, bid: nil, ask: nil },
        { provider: "BNM", date: "2026-01-02", base: "SDR", quote: "MYR", mid: 5.36, bid: nil, ask: nil },
        { provider: "BNM", date: "2026-01-02", base: "XDR", quote: "MYR", mid: nil, bid: 5.35, ask: 5.37 },
      ])
      DB[:blended_rates].insert(date: "2026-01-01", quote: "MYR", rate: 4.4)
      ...
      abort "migration marked complete" unless DB[:schema_info].get(:version) == 38
      abort "invalidated blends after failure" unless DB[:blended_rates].count == 1
      DB.disconnect
    RUBY
  end

  it "leaves databases without BNM SDR history alone" do
    run_migration_script(<<~RUBY)
      DB[:blended_rates].insert(date: "2026-01-01", quote: "MYR", rate: 4.4)
      Sequel::Migrator.run(DB, "db/migrate")
