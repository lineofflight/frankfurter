...
- V2 latest and single-date queries are now served from the materialized blend table and carry canonical values: a carried-forward row freezes at the value blended on its own observation date. Previously the blend was recomputed relative to the asking day, so a stale currency's latest value kept drifting as unrelated currencies published (the same dated row could read 10.30 in a range but 10.11 when asked five days later, both stamped with the same observation date). A dated row now means one thing everywhere: latest, single-date, and range responses agree by construction. Relatedly, the live-computed snapshots that remain (`providers=`, `expand=providers`, and installations whose table is not yet materialized) now always blend through the USD pivot like ranges; previously a snapshot whose contributing rows all shared the requested base blended directly in that base, producing slightly different values for the same data depending on the request shape. (#573)
...
- Restored Central Bank of Egypt (CBE) rates, stalled since 2026-07-22. The bank's WAF started rejecting requests that carry no Accept header, answering HTTP 200 with a "Request Rejected" page in place of the historical-data form; the adapter found no verification token there and aborted on every run. Adapters now send `Accept: */*` by default, since a request without the header reads as a bot rather than as a neutral "anything", closing the 28-day gap. (#576)
- Restored Banca Națională a României (BNR) rates, stalled since 2026-08-05 after the bank relaunched its site and moved the XML rate feeds to a dedicated curs.bnr.ro subdomain. The old www.bnr.ro paths now redirect to the homepage, which the adapter correctly refused to parse. The adapter fetches the yearly archives from their new host, closing the 10-day gap. (#580)
- Central Bank of Kenya (CBK) East African cross rates are now stored as published instead of inverted at ingest. CBK's table mixes two orientations: most rows are KES per foreign unit, but UGX, TZS, RWF and BIF are published foreign per KES. The adapter used to invert those four so every row read base foreign, quote KES, which turned an exact published figure into a repeating decimal (`24.0657` became `0.0343951103911`) and stored it. Single-provider responses echo stored digits verbatim, so `?providers=CBK` showed twelve digits of manufactured precision where the source publishes four. Those pairs now sit as base KES, quote foreign, and `?providers=CBK&base=KES` echoes `24.0657` exactly; any other orientation is computed at query time and rounded on output. 18,662 inverted rows spanning 2003-09-12 onward are dropped by migration and replaced by a re-backfill. Blended responses are unaffected: they round on output, and both orientations carry the same value. (#585)
- Single-provider V2 responses no longer show binary float noise in the low digits. `?providers=CBLLR&date=2026-08-11` returned `181.64319999999998` for USD/LRD where the source's own buy and sell prices average to exactly `181.6432`. These shapes deliberately echo a provider's published precision rather than rounding, so any noise stored at ingest reached the response verbatim. Adapters synthesize values the source never published (buy/sell midpoints, per-unit rescaling, cross rates) and stored the raw result of float arithmetic; rates are now rounded to twelve significant digits on the way in, which sits above every digit a provider actually publishes and below every digit the arithmetic invents. Adapters that coerce buy/sell to a mid also compute it in decimal now, so the mid is exact rather than merely clean. A migration applies the same rounding to the 690,000 already-stored rows that exceeded it, across 46 providers; for 598,000 of them the stored value was a short decimal wearing a binary tail, and those are now exact. Blended and grouped responses were never affected: they round on output. (#579)
- Recovered the precision Turkish central bank (TCMB) rates lost on ingest. The adapter rounded every rate to four decimal places, a fixed absolute precision applied across rates spanning four orders of magnitude: six significant digits for USD/TRY, but only one for JPY/TRY in the pre-redenomination era, where two years of daily movement collapsed into ten distinct values (sixty consecutive trading days reported at 0.0007 through a period when the lira was depreciating continuously). 192 stored rows kept a single significant digit and 1,379 kept two. The rounding also ran after the per-100 division that JPY needs, so JPY alone was rounded in the divided-down scale while every other currency was rounded in the source's own. It clipped modern magnitudes too: a buy of 28.0072 and a sell of 28.1927 average to 0.2809995 per yen and were stored as 0.281. The round dated from before #579, when it served as float-noise control; that job now belongs to the exact decimal mid and to RatePrecision at ingest, so the round is gone and the full mid is stored. Bank Indonesia (BI) carried the same rounding and loses it too, though IDR magnitudes meant it never bit. TCMB history is re-backfilled from its coverage start to recover the lost digits, which no migration could repair. (#584)
