...
- V2 date ranges spanning more than 5 years at daily granularity now return 422 unless `quotes` narrows the response to at most 5 currencies. The error names the ways out: a `quotes` filter, `group=week` or `group=month` aggregation, or splitting the range into shorter requests. This cap is interim and will be lifted for plain daily ranges once blended rates are materialized. (#569, #570)
