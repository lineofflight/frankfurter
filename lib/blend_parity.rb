...
# Replays table-eligible query shapes through both the materialized-table path and the live compute path and compares
# serialized bytes. One declared divergence is asserted rather than ignored (#570): snap-back rows serve the canonical
# anchor-date value, so when a quote's contributor set changed between its observation date and the range start, the
# table row may differ from what the live path computes at the range-start anchor. Canonicality is asserted in the pivot
# frame: derive divides a whole batch by the base's rate at the range-start anchor (a batch property, identical on both
# paths and pinned by the byte-equal fresh rows around it), so only the row's own pivot-frame value distinguishes
...
class BlendParity
  ...
  def explain_divergence(shape, table, live)
    ...
    live.reject { |r| table_keys.include?(r.values_at(:date, :quote)) }.each do |r|
      # The table emits at most one pre-range row per quote (the newest canonical row at the range start); live can
  ...
  # The canonical value for a record is what a live range anchored at the record's own date computes, in the pivot frame
  # so no derive denominator muddies the comparison.
  def canonical_rate(shape, record)
    pivot_frame_rate(shape, record, from: record[:date], to: record[:date], force_live: true)
  end

  # The record's pivot-frame value over the given window, shaped like the original request but based on the pivot so the
