...
module Providers
  ...
  class CBC < Base
    ...
    EARLIEST_DATE = Date.new(1993, 1, 5)
    ...
    class << self
      def key = "CBC"
      def name = "Central Bank of the Republic of China (Taiwan)"
    ...
    def fetch(since: nil, upto: nil)
      ...
      @dataset = parse(rows, start_date, end_date)
      self
    ...
    def parse(rows, start_date = nil, end_date = nil)
      rows.each_with_object([]) do |row, result|
        ...
        COLUMNS.each do |index, (quote, base)|
          ...
          result << { provider: key, date:, base:, quote:, rate: }
        end
