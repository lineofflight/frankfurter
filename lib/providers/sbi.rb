...
module Providers
  ...
  class SBI < Base
    ...
    EARLIEST_DATE = Date.new(1981, 1, 1)
    ...
    def fetch(since: nil, upto: nil)
      ...
      xml9 = fetch_group(start_date, end_date, 9)
      xml7 = fetch_group(start_date, end_date, 7)
    ...
    def parse(xml, currency_map = GROUP9_CURRENCIES.merge(GROUP7_CURRENCIES))
      ...
      doc.locate("*/TimeSeries").each do |ts|
        ...
        ts.locate("TimeSeriesData/Entry").each do |entry|
          ...
          records << { provider: key, date:, base: currency, quote: "ISK", rate: }
