...
class Provider
  module Adapters
    # National Bank of the Kyrgyz Republic. Publishes daily rates for 5 major currencies (USD, EUR, RUB, KZT, CNY) and
    # weekly rates for ~35 others against KGS.
    #
    # Two-track adapter:
    #
    ...
    class NBKR < Adapter
      ...
      # (foreign side of "Nominal foreign = X KGS"). The mapping was extracted from the <select name="valuta_id"> on the
      # landing page and cross-checked against an allVals snapshot to confirm ISO codes. Some currencies appear twice
      # across a redenomination (e.g. BYR/BYN, RUR/RUB, AZM/AZN, TRL/TRY) — each id covers its own slice of history.
      CURRENCIES = [
      ...
      # Parses the per-currency historical HTML series. The page interleaves comment markers around each cell
      # (<!--date-->...<!--date-->, <!--value-->... <!--value-->) which we match pairwise.
