...
class Provider
  module Adapters
    # Banque Centrale de Tunisie. Publishes daily reference exchange rates for 20 currencies against the Tunisian dinar
    # (TND) on the interbank market.
    #
    # The endpoint accepts a single date per request and returns an HTML fragment with two tables: the interbank
    # reference rates ("Cours Moyens des Devises Cotees") and a manual-exchange table below. We parse only the first
    # table.
    #
    ...
    #
    # The bonus XML endpoint described in #368 returns dates but no values in current responses, so we use the HTML
    # endpoint for all 20 currencies.
    class BCT < Adapter
      ...
      # The HTML meta tag declares ISO-8859-1 but the HTTP Content-Type header currently reports UTF-8, and the bytes
      # are valid UTF-8. We trust whichever encoding gives valid characters, falling back to a transcode from ISO-8859-1
      # only if the body isn't valid UTF-8.
      def decode(response)
