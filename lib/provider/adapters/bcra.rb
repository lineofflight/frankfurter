...
class Provider
  module Adapters
    # Central Bank of Argentina (Banco Central de la Republica Argentina). Fetches official exchange rates in ARS. The
    # API only accepts a single date per request, so we iterate day by day skipping weekends.
    class BCRA < Adapter
      ...
      def parse(json)
        ...
        unless fecha
          ...
          return [] if results.key?("fecha") && fecha.nil? && detalle.empty?
      ...
      # BCRA quotes some low-value currencies per N units, indicated in the descripcion field, e.g. "DONG VIETNAM
      # (C/1.000 UNIDADES)". Extract the multiplier so we can normalize to per-1-unit rates.
