...
Sequel.migration do
  up do
    ...
    transaction do
      legacy = from(:rates).where(provider: "RBM").where(Sequel.|({ base: "SDR" }, { quote: "SDR" }))
      old_code = Sequel.|({ base: "SDR" }, { quote: "SDR" })
      old_rollups = [:weekly_rates, :monthly_rates].any? do |table|
        !from(table).where(provider: "RBM").where(old_code).empty?
      end
      excluded = !from(:currency_exclusions).where(provider_key: "RBM", iso_code: "SDR").empty?
      ...
      rows.each do |row|
        ...
        if existing
          ...
          unless existing.values_at(*components) == row.values_at(*components)
            raise "RBM: conflicting SDR/XDR components on #{row[:date]} (#{row[:base]}/#{row[:quote]})"
          end
...

      {
        ...
        source = from(:rates).where(provider: "RBM").where(pairs)
        rollups = from(table).where(provider: "RBM").where(pairs)
      ...
      CurrencySummary.refresh(self, codes | counterparts | ["MWK"], provider: "RBM")
